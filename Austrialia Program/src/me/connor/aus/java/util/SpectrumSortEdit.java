 ...
 * @author Marc D'Avanzo, edidted by connor Clark
 */
...
public class SpectrumSortEdit {
	private int upperBoundIndex;
	private int lowerBoundIndex;
	private PartyArrayEdit parties;
	private String[] coalitionParties = { "LIBERAL/THE NATIONALS", "Liberal & Nationals",
	...
	// private double bestStandDev;
	// private String standDevParties;
	private double altSatndardDev;
	// private double altBestStandardDev;
	// private String altStandDevParties;

	/**
	 * @param none
	 * @return void
	 * @throws FileNotFoundException this constructor will set all the instance
	 *                               variables to their default values
	 */
	public SpectrumSortEdit(int uBound, int lBound) throws FileNotFoundException {
		...
		// bestStandDev = -1;
		altSatndardDev = 0;
		// altBestStandardDev = -1;
		// altStandDevParties = "";
		parties = new PartyArrayEdit(upperBoundIndex, lowerBoundIndex);
	...
	@Deprecated
	public SpectrumSortEdit() throws FileNotFoundException {
		// standDevParties = "";
		standardDev = 0;
		// bestStandDev = -1;
		altSatndardDev = 0;
	 ...
	 * @throws IOException 
	 */
	public void runPartyArray() throws IOException {
	...
	/**
	 * @param stateID
	 * @return ArrayList<String> This method will match the correct ballot order the
	 *         correct csv file
	 */
	...
	/**
	 * @param none
	 * @return void This method will find the indexes of the coalition and labor
	 *         parties for the current order
	 */
	public void getMajorPartyIndex() {
		...
		if (freshOrder) {
			...
			for (int x = 0; x < list.size(); x++) {
				...
				for (int cIndex = 0; cIndex < coalitionParties.length; cIndex++) {
					...
					if (list.get(x).equals(coalitionParties[cIndex])) {
						// System.out.println("Match: " + order.get(x) + " equals " +
						// coalitionParties[cIndex]);
			...
			/**
			 * for(int temp: coalitionIndex) { System.out.println("CoalitionIndex: " +
			 * temp); if(temp != -1) { System.out.println(order.get(temp)); } }
			 * System.out.println(""); System.out.println(""); for(int temp: laborIndex) {
			 * System.out.println("laborIndex: " + temp); if(temp != -1) {
	...
	/**
	 * @param partyName
	 * @return void This method will find the party's index.
	 */
	...
	/**
	 * @param none
	 * @return void This method will go go through every ballot in the party array
	 *         to calculate its spectrum value
	 */
	public void sortThrough() {
		...
		for (int partiesIndex = 0; partiesIndex < parties.getPartyArray().size(); partiesIndex++) {
			...
			// for(int ballotIndex = 0; ballotIndex < parties.getBallotSize(partiesIndex);
			// ballotIndex++ )
			// {
		}
	}
	...
	/**
	 * @param none
	 * @return void This method will print all the spectrumVals of every party
	 * @throws IOException
	 */
	public void printSpectrum() throws IOException {

		for (ManualParty party : parties.getPartyArray()) {
			party.addSpectrumVal();
		}
		calculateStandardDeviation();
		calcAltStandardDeviation();
		parties.printSpectrumVals(standardDev, altSatndardDev);
	}

	/*
	 * public void printBestStandDev() throws IOException { File file = new
	 * File("output\\BestStandDev.txt"); PrintWriter writer = new PrintWriter(new
	 * FileWriter(file, false)); try { if (file.createNewFile()) {
	 * System.err.println("stand dev file made"); } else {
	 * System.err.println("stand dev file exists"); } } catch (IOException e) {
	 * 
	 * } writer.println("Best Standard Deviation: "); writer.printf("%.4f\n",
	 * bestStandDev); writer.println("Case: " + standDevParties);
	 * writer.println("\n" + "Best Non-Extreme Standard Deviation: ");
	 * writer.printf("%.4f\n", altBestStandardDev); writer.println("Case: " +
	 * altStandDevParties); writer.close(); } /*
	 * 
	 * /** This method will calculate the standard deviation and print it
	 */
	public void calculateStandardDeviation() {
		boolean listSizeFound = false;
		int listSize = 0;
		while (!listSizeFound) {
			for (ManualParty party : parties.getPartyArray()) {
				double temp = party.getSpectrumVals();
				if (temp > -1) {
					listSize++;
				}
			}
		...
		// Step 5: divide by list size
		deviation /= list.length;

		// Step 6: Square root for final answer
	...
	/**
	 * Standard Deviaiton with no extremes (0 and 1)
	 */
	public void calcAltStandardDeviation() {
		boolean listSizeFound = false;
		int listSize = 0;
		while (!listSizeFound) {
			for (ManualParty party : parties.getPartyArray()) {
	...
	public double getAltStandardDeviation() {
		return altSatndardDev;
	}

	public static void main(String[] args) throws IOException {
		SpectrumSortEdit test = new SpectrumSortEdit();
		test.runPartyArray();
		test.sortThrough();
		test.printSpectrum();
